<template>
    ...
    <!-- 是否删除 -->
     <Modal
        ...
        <div v-if="!addAdStyle">创建时间 ：
          <Input v-model="adCreateTime" disabled></Input>
        </div>
        ...
        <p> 跳转地址：
          ...
          <span v-if="listenBannerUrl">
            ...
            <Icon type="close-round" v-else></Icon>
				...
				<p>广告备注：<Input v-model="adRemark" type="textarea" :rows="4" placeholder="添加广告备注..."></Input></p>
		  	</div>
...
export default {
            ...
            return h("div", [
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      adDetail({ serialNumber: obj.row.serialNumber }).then(
                        res => {
                          if (!res.code) {
                            this.addModal = true;
              ...
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      stickTopAdv({ serialNum: obj.row.serialNumber })
                        .then(res => {
                          if (!res.code) {
  ...
  methods: {
    ...
    confirmDel() {
      deleteAd({ ids: this.delArr }).then(res => {
        if (!res.code) {
          this.delArr = [];
    ...
    confrimAddAd(val) {
      ...
      if (judgeEmpty) {
        ...
        for (key in adObj) {
          if (key !== "remark") {
            // if (!String(adObj[key]) || !String(adObj[key]).length) pass = false;
